...
name <- "l100"
### NB: Values in 'iter' column must be equal and all scenarios must be distinct
...
for (n in 1:nrow(noise_df)){
  ...
  for (m in 1:length(noise_names)){ # Load stochasticity values
  ...
  l <- nrow(all_simu %>%
              dplyr::filter(scen==unique(all_simu$scen)[1] & iter==1))
...
# Make subsampling of stored timeseries ----------------------------------
